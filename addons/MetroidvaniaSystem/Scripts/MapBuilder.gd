...
## [br][br]Note that, while cells created with this class are part of [method MetroidvaniaSystem.get_save_data]'s dump, you can' differentiate them from regular cell overrides. You need to keep this information yourself.
extends RefCounted
...
## Requests the world map update. This simply emits [signal MetroidvaniaSystem.map_updated]. The signal is not emitted when customizing the cells, unlike regular overrides. Updating the map in batches is more efficient.
func update_map():
